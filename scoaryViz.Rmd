...
blastp <- read.table("/Users/hcm59/Box/Holly/Dog_E_coli_project/ecoli_acc_proteins_blastp_accVer_unique.out",header=FALSE)
colnames(blastp) <- c("qseqid","sseqid","sacc","sgi","qaccver","pident","length","mismatch","gapopen","qstart","qend","sstart","send","evalue","bitscore")
...
write.csv(gene_key, file="/Users/hcm59/Box/Goodman\ Lab/Projects/bacterial\ genomics/Ecoli_dog_AMR_results/dog_verified_host/Holly_Analyses/gene_key_")
...
giantMatrixAllAbxSig <- rbind(amikacin,amp,amox_clauv_acid,cefalexin.comb,cefazolin,cefpodoxime,chloram,doxy,enro,gentamicin,imipenem,marbo,orbi,pipTazo,tetracycline,trimeth_sulfameth)
...
giantMatrixAllAbxSigEmpBonfSml <- giantMatrixAllAbxSigEmpBonf[,c(1,3,10,12,19,20)]
...
library(dplyr)
giantMatrixAllAbxSigEmpBonfSmlCount <- giantMatrixAllAbxSigEmpBonfSml %>% group_by(Gene) %>% mutate(Count=n_distinct(Abx))
# yay! This is exactly what I want! 

library(ggplot2)
library(viridis)
heatmap <- ggplot(giantMatrixAllAbxSigEmpBonfSmlCount, aes(x = Abx, Gene)) +
  geom_tile(aes(fill = Count)) + theme_classic() + theme(axis.text.y = element_text(size = 2),axis.text.x = element_text(angle=45,vjust=0.5,size = 8)) # + scale_fill_viridis(option="magma",direction=-1)


pdf("/Users/hcm59/Box/Holly/Analyses/Analyses/Ecoli/Visualizations/heatmap1.pdf")
heatmap
dev.off()
...
ecoliMatrix <- giantMatrixAllAbxSigEmpBonfSmlCount[,c(1,2,6,7)]
...
data_wide <- dcast(ecoliMatrix, Gene + Annotation ~ Abx, value.var="Count")
data_wide


heatmap_data <- data_wide[,c(1,3:18)]
str(heatmap_data)
typeof(heatmap_data)
heatmap_data <- as.data.frame(heatmap_data)
typeof(heatmap_data)
heatmap_data_num <- heatmap_data %>% as.numeric(heatmap_data[,c(2:17)])
pheatmap(heatmap_data,main = "pheatmap default",cluster_rows = F, cluster_cols = F)

# now that we have that in order, let's actually go in and manually add in our annotations 
library(DataEditR)
data_wideAnno <- data_edit(data_wide,save_as = "ecoliMatrixWide_anno.txt",code="wideMatrixAnno.R")
data_edit(data_wide, )

testHeatmap <- pheatmap(data_wide, 
         color = diverging_hcl(7,palette = "Blue-Red 2"),
