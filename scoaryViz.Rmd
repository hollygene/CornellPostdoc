...
gene_key <- read.table("/Users/hcm59/Box/Goodman\ Lab/Projects/bacterial\ genomics/Ecoli_dog_AMR_results/dog_verified_host/Holly_Analyses/gene_id_key.txt",header=TRUE)
AMR_genes_NCBI <- read.csv("/Users/hcm59/Box/Goodman\ Lab/Projects/bacterial\ genomics/Ecoli_dog_AMR_results/dog_verified_host/Holly_Analyses/refgenes_NCBI_3.1.21.tsv",header=TRUE,sep="\t")
...
clavamox <- amox_clauv_acid %>% filter(Empirical_p < 0.05 & Bonferroni_p < 0.05) 
clavamox$log_or <- log10(clavamox$Odds_ratio)
clavamox_l <- c(clavamox$Gene)

# filter gene presence absence matrix to keep only genes significanctly
# associated with clavamox resistance
genepa_clavamox <- gene_pres_abs %>% filter(Gene %in% clavamox_l)
genepa_clavamox_combined <- left_join(genepa_clavamox, clavamox)
genepa_filtered <- genepa_clavamox[,15:ncol(genepa_clavamox)]
rownames(genepa_filtered) = genepa_clavamox$Gene
...
for (r in 1:nrow(genepa_filtered)){
  for (c in 1:ncol(genepa_filtered)){
    if (genepa_filtered[r,c] != ""){
      gene_mat[r,c]= genepa_clavamox_combined$log_or[r] 
    } else{
      gene_mat[r,c]= -2#0
    }
  }
...
pheno <- pheno %>% dplyr::select(X,Amoxicillin.Clavulanic.Acid.INT)
write.table(pheno, "clavamox_pheno_4itol.txt",quote=F, row.names = F)
