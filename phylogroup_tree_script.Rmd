...
colnames(phylogroupData.mod) <- c("org","org2","phylogroup")
join data together
metadataphylo <- left_join(phylogroupData.mod,region_dataSub, by = c("org"),keep=TRUE,copy=FALSE)
phyloRegionsComb <- fuzzy_left_join(clermonTypingPh,region_dataSub, by = c("org" = "Assembly"), match_fun = stri_detect_fixed)
...
treeRegions
treeClermRegions <- treeClerm + new_scale_fill() + geom_fruit(
    data=phyloRegionsComb,
    geom=geom_star,
    mapping=aes(y=org,fill=geo_group),
    starshape=13,
