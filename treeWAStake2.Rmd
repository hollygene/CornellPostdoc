...
phenAll <- read.csv("/Users/hcm59/Box/Holly/Analyses/Analyses/Ecoli/phenotypesDogs171819.csv",header=TRUE)
...
phen.sub.01.2.1 <- phen.sub.01.2 %>%  mutate_if(is.atomic,funs(str_replace(., "NI", "")))
phen.sub.01.2.2 <- phen.sub.01.2.1 %>%  mutate_if(is.atomic,funs(str_replace(., "NS", "")))
phen.sub.01.2.3 <- phen.sub.01.2.2 %>%  mutate_if(is.atomic,funs(str_replace(., "NOTSUS", "")))
phen.sub.01.2.4 <- phen.sub.01.2.3 %>%  mutate_if(is.atomic,funs(str_replace(., "NOMIC", "")))
View(phen.sub.01.2.4)
phen.sub.01.5 <- phen.sub.01.2.4 %>% mutate_all(na_if,"")

# need to add .1 to end of rownames to match the gene presence/absence matrix for scoary
phen.sub.01.5$Assembly <- paste0(phenAll$Assembly, ".1")


rownames(phen.sub.01.5) <- phen.sub.01.5$Assembly
rownames(phen.sub.01.5)
# View(phen.sub.01.3)
phen.sub.01.6 <- phen.sub.01.5[,3:104]
phenSubFilt <- Filter(function(x)!all(is.na(x)), phen.sub.01.6)


# remove columns with more than 80% NAs
phenSubFilt.1 <- phenSubFilt[, which(colMeans(!is.na(phenSubFilt)) > 0.05)]


write.csv(phenSubFilt,file="/Users/hcm59/Box/Goodman\ Lab/Projects/bacterial\ genomics/Ecoli_dog_AMR_results/dog_verified_host/dog_verified_host_PhenoForScoary.csv")
# View(phen.sub.01.4)
str(phen.sub.01.4)
...
Tree <- read.tree("/Users/hcm59/Box/Goodman\ Lab/Projects/bacterial\ genomics/Ecoli_dog_AMR_results/dog_verified_host/dog_verified_host_gb_snp_sites.fasta.treefile")
Tree$tip.label
Tree2 <- Tree
