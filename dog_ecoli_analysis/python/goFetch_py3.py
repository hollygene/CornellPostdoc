...
def removeGeneID(dic, listToRemove):
    ...
    for k,v in list(dic.items()):
        first_key=list(v[2].keys())[0] #save first geneID in genegroup
        previous_value=[v[2][first_key]]
        for key,value in list(v[2].items()):
            if key != first_key:
        ...
        with open(gffdir+item+'.gff', 'r') as gffFile:
            for line in gffFile:
                if not line.startswith('##'):
                    if line.startswith('gnl') or line.startswith('NZ') or line.startswith('gi') or line.startswith('MA') or line.startswith('MC') or line.startswith('MAT') or line.startswith('CP'):
                        line=line.split('\t')
                        line=line[-1].split(';')
                        ...
                        if 'gene' in locusID:
                            ...
                            try:
                                ...
                                ref=line[3]
                                for key, value in list(dic.items()):
                                    if locusID in list(value[2].keys()):
...
def getGOnumbers(dic):
    ...
    for geneGroup,value in list(dic.items()):
        for geneID, values in list(value[2].items()):
            for uniprotID, v in list(values[2].items()):
...
def printReport(dic):
    ...
    with open("report"+time.strftime("_%d_%m_%Y_%H%M")+".tsv",'w') as outfile:
        ...
        for GeneGroupID, general in list(dic.items()):
            geneGroupID=GeneGroupID
            annotation = general[0]
            otherAnnotation = general[1]
            for GeneID, geneInfo in list(general[2].items()):
                geneID=GeneID
                giNumber=geneInfo[0]
                refNumber=geneInfo[1]
                for UniprotID, geneOntology in list(geneInfo[2].items()):
                    uniprotID=UniprotID
                    component=''
                    process=''
                    function=''
                    for domain in geneOntology:
                        for domainName, term in list(domain.items()):
