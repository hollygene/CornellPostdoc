...
tree <- ape::read.tree("/Users/hcm59/Library/CloudStorage/Box-Box/Holly/Dog_E_coli_project/jan22/panarooResults/1.26.22/core_gene_alignment.aln-gb.treefile")
tree
...
table(metadata$org %in% tree$tip.label)
table( tree$tip.label %in%  metadata$org )
...
tree2 <- ggtree(tree, layout = "circular", branch.length = "none", size=0.2)  + theme_tree() + geom_fruit(
         data=loc,
         geom=geom_star,
         mapping = aes(x=COLST, y=`AST_data$org`, fill=COLST),
         size=1,
          starstroke=0,
          pwidth=0.1,
          inherit.aes = FALSE,
